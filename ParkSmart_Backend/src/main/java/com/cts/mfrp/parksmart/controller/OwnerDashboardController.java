...
public class OwnerDashboardController {
    ...
    /** POST get owner's approval request history */
    @PostMapping("/approvals/my")
    public ResponseEntity<java.util.List<SpaceApprovalDTO>> getMyApprovals(Authentication auth) {
        return ResponseEntity.ok(ownerDashboardService.getOwnerApprovalRequests(auth.getName()));
    }

    /** POST generate slots for public space */
    @PostMapping("/slots/generate")
    public ResponseEntity<String> generateSlots(
            @Valid @RequestBody GenerateSlotsRequestDTO req, Authentication auth) {
        adminService.generatePublicSlots(req, auth.getName());
        return ResponseEntity.ok("Slots generated");
    }
}
