...
export class LoginModalComponent {
  ...
  constructor(private auth: AuthService, private modal: ModalService, private router: Router) {}
  ...
  submit() {
    ...
    this.auth.login({ email: this.email, password: this.password, rememberMe: this.rememberMe }).subscribe({
      next: () => { this.loading = false; this.modal.close(); this.router.navigate(['/search']); },
