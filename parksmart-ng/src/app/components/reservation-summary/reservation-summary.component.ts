...
export class ReservationSummaryComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (!this.card || !this.slotIds.length) {
  ...
  extendHold() {
    ...
    if (this.extendDone) {
      this.extendError = 'Hold can only be extended once.';
      return;
    }
    this.extendError = '';
    this.bookingSvc.extendHold(this.summary.holdId).subscribe({
      ...
      error: err => {
        this.extendError = err?.error || 'Cannot extend hold. It may have already been extended.';
      }
    });
  }
  ...
  releaseAndGoBack() {
    const navigate = () => this.router.navigate(['/slots', this.card?.spaceId ?? 0], {
      state: { card: this.card, arriving: this.arriving, leaving: this.leaving }
    });
    if (this.summary?.holdId) {
      this.parkingSvc.releaseSlots(this.summary.holdId).subscribe(() => navigate());
    } else {
      navigate();
    }
  }
